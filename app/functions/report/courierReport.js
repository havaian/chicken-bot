      ...
      <table border="1" style="width:100%; border-collapse: collapse;">
        ...
        ${limitedDeliveredTo
          .map((delivery, rowIndex) => {
            let deliveryHtml = "";
            const paymentHtml = `<td style="text-align: center; vertical-align: middle; background-color: ${rowIndex % 2 === 0 ? '#f6f6f6' : '#ffffff'};" rowspan="${delivery.eggs.length}">${delivery.payment}</td>`;
            const debtHtml = `<td style="text-align: center; vertical-align: middle; background-color: ${rowIndex % 2 === 0 ? '#f6f6f6' : '#ffffff'};" rowspan="${delivery.eggs.length}">${delivery.debt}</td>`;
            delivery.eggs.forEach((egg, index) => {
